## 🎓 SELF UNIVERSITY — Complete Platform Overview

### 📚 Mini Courses
- **Content modules** with MDX markup — each module is a full-fledged page with text, visualizations, and interactive elements
- **Interactive components inside modules:**
  - `MemoryStackPhase` — building memory layout (Text, Data, BSS, Heap, Stack)
  - `NetworkStackPhase` — building network stack (HTTP → TCP → IP → Ethernet)
  - `ByteDissection` — packet byte dissection (Ethernet, IP, TCP, HTTP)
  - `HexDump` — packet hex dump visualization
  - `PreTest` — pre-assessment before studying the module
  - `StackBridge` — connection between Memory Stack and Network Stack
  - `StackThreadQuiz` — quiz on stacks and threads
  - `WireLevelFinalQuiz` — final module quiz (6 questions)
  - `CounterfactualQuestion` — "what if" thought experiments
  - `BuildStack` — building both stacks simultaneously (Memory + Network)
  - `BuildStackTheory` — theory with hints
- **Visualizations (Tooltips):**
  - `LIFOVisualization` — LIFO stack with push/pop
  - `EncapsulationVisualization` — packet encapsulation
  - `DecapsulationVisualization` — packet decapsulation
  - `StackPointerVisualization` — stack pointer (RSP)
  - `TCPVisualization` — TCP header structure
  - `IPVisualization` — IP header structure
  - `EthernetVisualization` — Ethernet frame
  - `ARPVisualization` — ARP protocol
  - `MTUVisualization` — MTU and fragmentation
  - `ThreadStackVisualization` — thread stacks
  - `InterruptStackVisualization` — interrupt stack
  - `OSIVisualization` — OSI model (7 layers)
  - `TermTooltip` — popup tooltips with visualizations (works inline within module text)
- **Course Roadmap** — visual course roadmap with progress tracking and lock mechanics (must complete previous module to unlock next)
- **Module Notes** — personal notes for each module (private, user-only)
- **Bookmarks** — module bookmarks (save to revisit later)
- **Complete Module Button** — "Mark as Complete" button that awards XP
